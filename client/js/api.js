...
export async function downloadTaskExport(format) {
  const response = await fetch(`${API_BASE_URL}/reports/tasks/${format}`, {
    credentials: "include",
  });
...
export async function getReportStatus(jobId) {
  return request(`/reports/jobs/${jobId}`);
}
