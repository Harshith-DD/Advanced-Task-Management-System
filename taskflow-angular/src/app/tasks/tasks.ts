  ...
  saveTask(value: TaskFormSubmit): void {
    ...
    request.pipe(finalize(() => this.isSaving.set(false))).subscribe({
      next: response => {
        ...
        const assignment = value.assignedTo;

        const assignmentRequest = assignment !== null || !editing
          ? this.taskService.assignTask(taskId, assignment)
          : null;
        ...
        assignmentRequest.subscribe({
          ...
          error: error => {
            ...
            this.taskState.setError(
              error?.error?.error?.message ?? 'Task saved, but assignment failed.'
            );
          }
        });
      },
