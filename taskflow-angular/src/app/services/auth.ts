...
interface RegisterResponse { success: boolean; data: User; }
interface CurrentUserResponse { success: boolean; data: { user: User }; }
interface LogoutResponse { success: boolean; message: string; }
...
export class AuthService {
  ...
  restoreSession(): Observable<User> {
    ...
    if (!this.sessionRestore$) {
      ...
      this.sessionRestore$ = this.getCurrentUser().pipe(
        tap({
